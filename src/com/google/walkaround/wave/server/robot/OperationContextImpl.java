...
class OperationContextImpl implements OperationContext, OperationResults {
  ...
  public OpBasedWavelet openWavelet(WaveId waveId, WaveletId waveletId, ParticipantId participant)
    ...
    if (wavelet == null) {
      ...
      WaveletAndDeltas waveletAndDeltas = getWaveletAndDeltas(waveletName, participant);
      if (waveletAndDeltas == null) {
        ...
        ReadableWaveletData snapshot = waveletAndDeltas.getSnapshotAfterDeltas();
  ...
  public OpBasedWavelet openWavelet(OperationRequest operation, ParticipantId participant)
    ...
    try {
      WaveId waveId =
          ApiIdSerializer.instance().deserialiseWaveId(
              OperationUtil.<String>getRequiredParameter(operation, ParamsProperty.WAVE_ID));
      WaveletId waveletId =
          ApiIdSerializer.instance().deserialiseWaveletId(
  ...
  public ObservableConversationView openConversation(
      WaveId waveId, WaveletId waveletId, ParticipantId participant)
      throws InvalidRequestException {
    WaveletName waveletName = WaveletName.of(waveId, waveletId);
    LOG.info("Opening conversation for: " + waveletName);
    ...
    LOG.info("Returning " + conversations.get(participant));
